...
export class EngineValidator {
  ...
  public validatePrompt(
    ...
    try {
      this.logger.debug('🔍 [EngineValidator] Validating prompt', {
      ...
      this.logger.debug('✅ [EngineValidator] Prompt validation completed', {
        promptId: convertedPrompt.id,
        isValid,
        score,
      ...
      this.logger.error('❌ [EngineValidator] Prompt validation failed', {
        promptId: convertedPrompt.id,
  ...
  public async validateWithGates(
    ...
    try {
      this.logger.debug('🚪 [EngineValidator] Validating with gates', {
        promptId: convertedPrompt.id,
      ...
      this.logger.debug('✅ [EngineValidator] Gate validation completed', {
        promptId: convertedPrompt.id,
        allPassed,
      ...
      this.logger.error('❌ [EngineValidator] Gate validation failed', {
        promptId: convertedPrompt.id,
        error: error instanceof Error ? error.message : String(error),
      });
