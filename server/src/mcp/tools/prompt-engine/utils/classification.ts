...
export class PromptClassifier {
  ...
  public classifyPrompt(
    ...
    try {
      this.logger.debug('🔍 [PromptClassifier] Classifying prompt', {
      ...
      this.logger.debug('✅ [PromptClassifier] Prompt classified successfully', {
        promptId: convertedPrompt.id,
      ...
      this.logger.error('❌ [PromptClassifier] Prompt classification failed', {
        promptId: convertedPrompt.id,
  ...
  private performClassification(
    ...
    if (this.semanticAnalyzer) {
      try {
        ...
        this.logger.warn('⚠️ [PromptClassifier] Semantic analysis failed', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }
